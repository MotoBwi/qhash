...
from qhash.istftnet import Decoder
from qhash.modules import CustomAlbert, ProsodyPredictor, TextEncoder
...
class QModel(torch.nn.Module):
    """
    QModel is a torch.nn.Module with 2 main responsibilities:
    1. Init weights, downloading config.json + model.pth from HF if needed
    2. forward(phonemes: str, ref_s: FloatTensor) -> (audio: FloatTensor)

    You likely only need one QModel instance, and it can be reused across
    multiple QPipelines to avoid redundant memory allocation.

    Unlike QPipeline, QModel is language-blind.

    QModel stores self.vocab and thus knows how to map phonemes -> input_ids,
    so there is no need to repeatedly download config.json outside of QModel.
    """
    ...
    def __init__(
        ...
        if not model:
            model = hf_hub_download(
                repo_id=repo_id, filename=QModel.MODEL_NAMES[repo_id]
            )
    ...
    ) -> Union["QModel.Output", torch.FloatTensor]:
        input_ids = list(
...
class QModelForONNX(torch.nn.Module):
    def __init__(self, QModel: QModel):
        super().__init__()
        self.QModel = QModel
    ...
    def forward(
        ...
        waveform, duration = self.QModel.forward_with_tokens(input_ids, ref_s, speed)
