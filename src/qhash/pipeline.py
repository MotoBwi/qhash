...
class QPipeline:
    """
    QPipeline is a language-aware support class with 2 main responsibilities:
    ...
    You are expected to have one QPipeline per language. If you have multiple
    QPipelines, you should reuse one QModel instance across all of them.

    QPipeline is designed to work with a QModel, but this is not required.
    There are 2 ways to pass an existing model into a pipeline:
    1. On init: us_pipeline = QPipeline(lang_code='a', model=model)
    2. On call: us_pipeline(text, voice, model=model)

    By default, QPipeline will automatically initialize its own QModel. To
    suppress this, construct a "quiet" QPipeline with model=False.

    A "quiet" QPipeline yields (graphemes, phonemes, None) without generating
    any audio. You can use this to phonemize and chunk your text in advance.

    A "loud" QPipeline _with_ a model yields (graphemes, phonemes, audio).
    """
    ...
    def __init__(
        ...
        model: Union[QModel, bool] = True,
        trf: bool = False,
        en_callable: Optional[Callable[[str], str]] = None,
        device: Optional[str] = None,
    ):
        """Initialize a QPipeline.

        Args:
            lang_code: Language code for G2P processing
            model: QModel instance, True to create new model, False for no model
        ...
        if isinstance(model, QModel):
            ...
            if device is None:
                if torch.cuda.is_available():
                    ...
                    device = "cpu"
            try:
                self.model = QModel(repo_id=repo_id).to(device).eval()
    ...
    def waterfall_last(
        ...
        for w in waterfall:
            ...
            if z < len(tokens) and tokens[z].phonemes in bumps:
                z += 1
            if next_count - len(QPipeline.tokens_to_ps(tokens[:z])) <= 510:
                return z
        return len(tokens)
    ...
    def en_tokenize(
        ...
        for t in tokens:
            ...
            if next_pcount > 510:
                z = QPipeline.waterfall_last(tks, next_pcount)
                text = QPipeline.tokens_to_text(tks[:z])
                logger.debug(
                    f"Chunking text at {z}: '{text[:30]}{'...' if len(text) > 30 else ''}'"
                )
                ps = QPipeline.tokens_to_ps(tks[:z])
                yield text, ps, tks[:z]
                tks = tks[z:]
                pcount = len(QPipeline.tokens_to_ps(tks))
                if not tks:
                    next_ps = next_ps.lstrip()
            tks.append(t)
            pcount += len(next_ps)
        if tks:
            text = QPipeline.tokens_to_text(tks)
            ps = QPipeline.tokens_to_ps(tks)
    ...
    def generate_from_tokens(
        ...
        voice: str,
        speed: float = 1,
        model: Optional[QModel] = None,
    ) -> Generator["QPipeline.Result", None, None]:
        ...
        Args:
            ...
            model: Optional QModel instance (uses pipeline's model if not provided)

        Yields:
            QPipeline.Result containing the input tokens and generated audio
        ...
        if isinstance(tokens, str):
            ...
            output = QPipeline.infer(model, tokens, pack, speed) if model else None
        ...
        for gs, ps, tks in self.en_tokenize(tokens):
            if not ps:
                ...
                ps = ps[:510]
            output = QPipeline.infer(model, ps, pack, speed) if model else None
            if output is not None and output.pred_dur is not None:
                QPipeline.join_timestamps(tks, output.pred_dur)
    ...
    class Result:
        ...
        output: Optional[QModel.Output] = None
